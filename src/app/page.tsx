      ...
      <section className="bg-slate-900 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl text-slate-300 max-w-2xl mx-auto mb-10">
            Stop reading boring tutorials. Experience software design patterns through interactive simulations, real-world analogies, and deep insights.
          </p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {patterns.map((pattern) => (
            <Link key={pattern.id} href={`/patterns/${pattern.slug}`} className="group">
              <article className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all h-full flex flex-col">
                <div className="mb-4">
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide
                    ${pattern.type === 'Behavioral' ? 'bg-orange-100 text-orange-700' :
                      pattern.type === 'Structural' ? 'bg-purple-100 text-purple-700' :
